...
# ************### layout.py parameter change to : node, indent ***********####
def paint_tree(node, indent):
    indent.extend(node.paint())

    for child in node.children:
        paint_tree(child, indent)
...
# ************### layout.py change slant to style  ***********####
...
DEFAULT_STYLE_SHEET = CSSParser(open("browsersty.css").read()).parse()
...
class BlockLayout:
    ...
    def layout(self):
        ...
        if mode == "block":
            ...
            self.size = int(float(self.node.style["font-size"][:-2]))
    ...
    def open_tag(self, tag):
        if tag in ["i", "em"]:
            ...
            self.size = max(6, self.size - 2)
        elif tag == "big":
            self.size += 4
        elif tag == "br":
            self.flush()
        elif tag.startswith("h") and len(tag) == 2:
            self.size += (6 - int(tag[1])) * 2
            self.weight = "bold"
...
class Browser:
    def __init__(self):
        ...
        self.display_list = []
        self.default_style_sheet = self.load_css("/home/angkul/my_data/coding/browser/browsersty.css")

    def load_css(self, filename):
        with open(filename, "r") as f:
            return CSSParser(f.read()).parse()
